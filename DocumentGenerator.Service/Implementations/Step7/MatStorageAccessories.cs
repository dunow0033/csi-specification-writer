...
{
    public class MatStorageAccessories : IMatStorageAccessories
    {
        private readonly string MAT_STORAGE_ACCESSORIES = "MatStorageAccessories";

        private readonly IExcel _excel;

        public MatStorageAccessories(IExcel excel)
        {
            _excel = excel;
        }

        public int Create(string path, Models.Step7.MatStorageAccessories matStorageAccessories)
        {
            int maxId;
            var inputFile = new FileInfo(path);
            using (FastExcel.FastExcel fastExcel = new(inputFile))
            {
                Worksheet workSheet = fastExcel.Read(MAT_STORAGE_ACCESSORIES);
                ...
                fastExcel.Write(workSheet, MAT_STORAGE_ACCESSORIES);
            }

            return maxId;
        }

        public Models.Step7.MatStorageAccessories? GetByRow(Row row)
        {
            Models.Step7.MatStorageAccessories item = new();
        ...
        public int Update(string path, Models.Step7.MatStorageAccessories matStorageAccessories)
        {
