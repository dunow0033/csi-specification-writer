...
{
    public class VolleyballEquipmentAccessories : IVolleyballEquipmentAccessories
    {
        private readonly string VOLLEYBALL_EQUIPMENT_ACCESSORIES = "VolleyballEquipmentAccessories";

        private readonly IExcel _excel;

        public VolleyballEquipmentAccessories(IExcel excel)
        {
            _excel = excel;
        }

        public int Create(string path, Models.Step6.VolleyballEquipmentAccessories volleyballEquipmentAccessories)
        {
            int maxId;
            var inputFile = new FileInfo(path);
            using (FastExcel.FastExcel fastExcel = new(inputFile))
            {
                Worksheet workSheet = fastExcel.Read(VOLLEYBALL_EQUIPMENT_ACCESSORIES);
                ...
                fastExcel.Write(workSheet, VOLLEYBALL_EQUIPMENT_ACCESSORIES);
            }

            return maxId;
        }

        public Models.Step6.VolleyballEquipmentAccessories? GetByRow(Row row)
        {
            Models.Step6.VolleyballEquipmentAccessories item = new();
        ...
        public int Update(string path, Models.Step6.VolleyballEquipmentAccessories volleyballEquipmentAccessories)
        {
