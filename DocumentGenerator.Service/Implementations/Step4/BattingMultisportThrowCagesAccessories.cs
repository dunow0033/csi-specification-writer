...
{
    public class BattingMultisportThrowCagesAccessories : IBattingMultisportThrowCagesAccessories
    {
        private readonly string BATTING_MULTISPORT_THROW_CAGES_ACCESSORIES = "BattingMultiSportThrowCagesAcce";

        private readonly IExcel _excel;

        public BattingMultisportThrowCagesAccessories(IExcel excel)
        {
            _excel = excel;
        }

        public int Create(string path, BattingMultiSportThrowCagesAccessories battingMultiSportThrowCagesAccessories)
        {
            int maxId;
            var inputFile = new FileInfo(path);
            using (FastExcel.FastExcel fastExcel = new(inputFile))
            {
                Worksheet workSheet = fastExcel.Read(BATTING_MULTISPORT_THROW_CAGES_ACCESSORIES);
                ...
                fastExcel.Write(workSheet, BATTING_MULTISPORT_THROW_CAGES_ACCESSORIES);
            }

            return maxId;
        }

        public BattingMultiSportThrowCagesAccessories? GetByRow(Row row)
        {
            BattingMultiSportThrowCagesAccessories item = new();
        ...
        public int Update(string path, BattingMultiSportThrowCagesAccessories battingMultiSportThrowCagesAccessories)
        {
